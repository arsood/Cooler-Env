...
Other `lib/` helpers: `paths.ts` (`resolvePaths`, `validateEnvName`, `requireEnv`, `configPathOf` — env names are validated against a conservative file-name pattern in both the CLI and `loadEnv`, and `-p` is resolved with `path.resolve` so absolute paths work), `guards.ts` (`assertInitialized`, `validateKeyName`), `constants.ts` (the shared `DANGEROUS_KEYS` prototype-pollution list, used by both the validator and `sanitize`), `errors.ts` (`CoolerEnvError`), `banner.ts` (inlined ASCII art — no `figlet` dependency), `usage.ts` (`getVersion`, `helpText`), `dotenv.ts` (`formatEnvValue` — bare-or-JSON-quoted `KEY=value` rendering, used by `list --values`), `types.ts`.
