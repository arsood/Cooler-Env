...
1. **CLI** (`src/cli.ts`, the `bin`): shebang, banner (TTY only), and a single top-level `.catch` that prints `CoolerEnvError` messages to stderr and sets a non-zero exit code; everything else prints with a stack. Parsing and dispatch live in `src/run.ts` (`run(args)`) so tests can drive the CLI without spawning a process. `minimist` is given `string: ["e", "p"]` (so `-e`/`-p` are never coerced to numbers/booleans) plus boolean flags (`show`, `values`, `help`, `version`) with an `-h` alias, and string options `-k`/`--key` and `-v`/`--value` carrying `add`'s key/value for non-interactive use (so `-v` is **not** a `--version` alias); commands are looked up as own properties (so `constructor` is not a command). `run` handles `--version`/`--help` before dispatch, warns on any option outside `KNOWN_OPTIONS`, then dispatches. Commands: `init`, `add`, `edit`, `delete`, `list`, `export`. Every command takes `-e <env>` (required) and `-p <path>` (optional); `--help`/`--version` text lives in `src/lib/usage.ts`.
...
The on-disk `.yml.enc` format is a single binary blob with a versioned header: `[magic "CENV"(4)][version(1)][salt(16)][iv(12)][authTag(16)][ciphertext]`. A fresh random salt + IV is generated per write, the AES key is derived from the secret key via `scrypt(password, salt)`, and GCM's auth tag means a wrong key or any tampering fails loudly (throws `CoolerEnvError`) rather than yielding garbage. The header (added in v4) lets the KDF/cipher params evolve; it is bound into the GCM auth tag via `setAAD` (so the version byte can't be tampered with — don't drop that), and `decryptSecrets` reads headerless v3 blobs too (treated as version 0, with a fallback path that also rescues the ~1-in-2³² case where a v3 salt happens to start with the magic bytes), so v4 reads v3 files but not vice-versa. **This format is not backward-compatible with the old cryptify/aes-256-cbc files (v2.x); v3 is a hard break.**

Other `lib/` helpers: `paths.ts` (`resolvePaths`, `validateEnvName`, `requireEnv`, `configPathOf` — env names are validated against a conservative file-name pattern in both the CLI and `loadEnv`, and `-p` is resolved with `path.resolve` so absolute paths work), `guards.ts` (`assertInitialized`, `validateKeyName`, and the exported `KEY_NAME_PATTERN` reused by `export --shell`), `constants.ts` (the shared `DANGEROUS_KEYS` prototype-pollution list, used by both the validator and `sanitize`), `errors.ts` (`CoolerEnvError`), `banner.ts` (inlined ASCII art — no `figlet` dependency), `usage.ts` (`getVersion`, `helpText`), `dotenv.ts` (value renderers: `formatDisplayValue` — bare-or-JSON-quoted, a _display_ format for `list --values`, **not** safe to source; `formatDotenvValue` — bare / single-quoted / lossy double-quoted `.env` tiers, returns `{ text, lossy }`; `formatShellValue` — POSIX single-quoting that round-trips everything, for `export --shell`), `types.ts`.

### Tests

Jest (`ts-jest` for `.ts`, `babel-jest` for `.js`). Tests import commands directly from `src/`, run inside an isolated temp dir via `test/sandbox.ts` (`makeSandbox` chdirs into a realpath'd `os.tmpdir()` subdir so nothing touches the real repo or its `.gitignore`, and so `sandbox.dir` equals `process.cwd()` on macOS where `/var` is a symlink; it also drops a bare `.git` marker dir so `init` sees the sandbox as a repo root and stays quiet about the missing-repo warning, and sets `GIT_CEILING_DIRECTORIES` to the sandbox's parent so `init`'s walk up the tree can't reach a real `.git` above the temp dir — tests needing a real repo run their own `git init`), and mock `inquirer.prompt`. Always assert without swallowing errors — the original init test wrapped everything in try/catch and passed even when `init` threw. Coverage is enforced in CI (`yarn test:coverage`) with a `coverageThreshold` floor in `jest.config.js` (90% statements/lines, 85% branches/functions, over `src/` minus the thin `cli.ts` wrapper).
