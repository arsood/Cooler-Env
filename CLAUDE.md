...
Jest (`ts-jest` for `.ts`, `babel-jest` for `.js`). Tests import commands directly from `src/`, run inside an isolated temp dir via `test/sandbox.ts` (`makeSandbox` chdirs into a realpath'd `os.tmpdir()` subdir so nothing touches the real repo or its `.gitignore`, and so `sandbox.dir` equals `process.cwd()` on macOS where `/var` is a symlink; it also drops a bare `.git` marker dir so `init` sees the sandbox as a repo root and stays quiet about the missing-repo warning — tests needing a real repo run their own `git init`), and mock `inquirer.prompt`. Always assert without swallowing errors — the original init test wrapped everything in try/catch and passed even when `init` threw. Coverage is enforced in CI (`yarn test:coverage`) with a `coverageThreshold` floor in `jest.config.js` (currently 90% statements/functions/lines, 85% branches, over `src/` minus the thin `cli.ts` wrapper).
